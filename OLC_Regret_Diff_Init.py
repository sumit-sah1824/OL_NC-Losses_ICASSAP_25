...
for dist in diff_init:
    ...
    # dist = diff_init # scaling for the offset
    ...
    # variation = []
    # for i in range(len(cumulative_losses_100_rounds)):
    #     var = cumulative_losses_100_rounds[i+1]-cumulative_losses_100_rounds[i]
    #     # print(var)
...
for idx, rad in enumerate(radii):
    plt.plot(range(1, len(dynamic_regrets_zero[idx]) + 1), dynamic_regrets_zero[idx], label=rf'$\rho$ = {rad:.2f}$(w_0)$')
    plt.plot(range(1, len(dynamic_regrets_minus_10[idx]) + 1), dynamic_regrets_minus_10[idx], label=rf'$\rho$ = {rad:.2f}$(w_1)$')
