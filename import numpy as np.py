...
x_train = x_train[:50000].reshape(-1, 32*32*3).astype('float32') / 255.0  # Flatten images
y_train = to_categorical(y_train[:50000], 10)

# Step 2: Prepare batches with 70% repeated data and 30% unique data
batch_size = 32
num_batches = len(x_train) // batch_size
repeat_ratio = 0.70

# Step 2.1: Select 70% of the dataset to repeat in every batch
num_repeated = int(batch_size * repeat_ratio)
x_repeated = x_train[:num_repeated]
y_repeated = y_train[:num_repeated]

# Step 2.2: Prepare batches with 70% repeated data and 30% unique data
x_batches = []
y_batches = []
